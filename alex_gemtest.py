...
class alexis_gemini(loader.Module):
    ...
    async def ghist(self, message):
        ...
        clean_result = html.escape(result)
        clean_joke = html.escape(joke)
    ...
    async def generate_joke(self, chat_text):
        ...
        try:
            ...
            content_parts = [genai.protos.Part(text=joke_prompt)]
            response = model.generate_content(content_parts)
            joke_text = response.text.strip() if response.text else "Шутка не сгенерирована."

            return joke_text
        except Exception as e:
            return "Не получилось сгенерировать шутку."
